#!/usr/bin/env python3
"""Inject Hippo OS species-specific behaviour clips into model specs.

The base JSON files define anatomy and core locomotion. This build step adds richer
behaviour clips deterministically before anyCreature compiles the production GLBs.
"""
...
EXTRA_ANIMATIONS = {
    "hippo.json": {
        ...
        "wallow": {
            ...
            "loop": True,
            "tracks": {
                "Hips": {"ty": keyframes((0, -0.015), (0.25, -0.050), (0.55, -0.025), (0.82, -0.055), (1, -0.015)),
                         "ry": keyframes((0, -2.0), (0.35, 4.0), (0.70, -4.0), (1, -2.0))},
                "NeckB": {"rx": keyframes((0, 5.0), (0.35, 11.0), (0.68, 6.0), (1, 5.0)),
    ...
    "shar_pei.json": {
        "observe": {
            ...
            "loop": True,
            "tracks": {
                "NeckB": {"ry": keyframes((0, -8.0), (0.30, 2.0), (0.58, 10.0), (0.82, 1.0), (1, -8.0)),
...
def augment(path: Path, additions: dict) -> None:
    ...
    print(f"{path.name}: animations={','.join(animations.keys())}")
