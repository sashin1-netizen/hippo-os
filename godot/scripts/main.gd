...
func _make_rocks_and_plants() -> void:
    var rock_positions := [
        Vector3(-5.8, 0.45, -3.8), Vector3(5.7, 0.35, -4.2),
        Vector3(-6.6, 0.32, 4.7), Vector3(6.2, 0.42, 4.8)
    ]
...
func _choose_action() -> void:
    ...
    var scores := {
        ...
        "approach": (0.18 + bond * 0.48 + affection * 0.18) * lerp(0.72, 1.45, personality["affection"]),
        "sleep": (1.0 - energy) * 1.08,
        "explore": ((1.0 - curiosity) * 0.66 + energy * 0.20) * lerp(0.75, 1.42, personality["curiosity"]),
        "play": (energy * 0.34 + curiosity * 0.25 + bond * 0.16) * lerp(0.74, 1.50, personality["energy"])
    }
    var best := "idle"
    var best_score := -1.0
    for key in scores.keys():
        var score: float = scores[key] + randf_range(-0.035, 0.035)
        if score > best_score:
            best_score = score
            best = key
    current_action = best
...
func _update_hippo(delta: float) -> void:
    ...
    var breathing := sin(t * 2.2) * 0.025
    hippo_visual.scale.y = 1.0 + breathing

    var flick := sin(t * 5.1) * 0.18
    ear_l.rotation.z = -0.2 + flick
    ear_r.rotation.z = 0.2 - flick

    if pet_pulse > 0.0:
        pet_pulse = max(0.0, pet_pulse - delta * 2.2)
        head.rotation.z = sin(t * 10.0) * 0.12 * pet_pulse
    else:
        head.rotation.z = lerp(head.rotation.z, 0.0, delta * 6.0)

func _feed_hippo() -> void:
    hunger = clamp(hunger - 0.28, 0.0, 1.0)
    affection = clamp(affection + 0.035, 0.0, 1.0)
    ...
    if event is InputEventScreenTouch:
        ...
        if touch_on_hippo:
            if _screen_hits_hippo(event.position):
                pet_distance += delta.length()
                if pet_distance >= 42.0:
                    _pet(clamp(pet_distance / 120.0, 0.4, 1.5))
                    pet_distance = 0.0
        else:
            orbit_yaw -= delta.x * 0.006
            orbit_pitch = clamp(orbit_pitch - delta.y * 0.004, -0.55, 0.20)
        last_touch = event.position
        ...
        if touch_on_hippo:
            pet_distance += event.relative.length()
            if pet_distance >= 42.0:
                _pet(clamp(pet_distance / 120.0, 0.4, 1.5))
                pet_distance = 0.0
        else:
            orbit_yaw -= event.relative.x * 0.006
            orbit_pitch = clamp(orbit_pitch - event.relative.y * 0.004, -0.55, 0.20)

func _screen_hits_hippo(screen_pos: Vector2) -> bool:
    if not camera or not hippo:
        return false
    ...
    return not hit.is_empty() and hit.get("collider") == hippo
...
func _update_camera() -> void:
    ...
    var offset := Vector3(
        sin(orbit_yaw) * cp,
        sin(-orbit_pitch) + 0.42,
        cos(orbit_yaw) * cp
    ) * orbit_distance
    camera.global_position = orbit_pivot + offset
    camera.look_at(orbit_pivot + Vector3(0.0, 0.35, 0.0), Vector3.UP)

func _update_ui() -> void:
    if not title_label:
        return
    title_label.text = "HIPPO OS  •  %s" % hippo_name
    stats_label.text = "Bond %d%%   Hunger %d%%   Energy %d%%" % [roundi(bond * 100.0), roundi(hunger * 100.0), roundi(energy * 100.0)]
    action_label.text = "Mood: %s   •   Pets: %d   •   Feeds: %d" % [current_action.capitalize(), interaction_counts.get("pet", 0), interaction_counts.get("feed", 0)]
...
func _load_state() -> void:
    ...
    hippo_name = str(parsed.get("name", hippo_name))
    ...
    interaction_counts = parsed.get("interactions", interaction_counts)
    session_count = int(parsed.get("sessions", 0)) + 1
    last_save_unix = int(parsed.get("saved_at", Time.get_unix_time_from_system()))

    var now := int(Time.get_unix_time_from_system())
    var offline_minutes := clamp(float(now - last_save_unix) / 60.0, 0.0, 4320.0)
    hunger = clamp(hunger + 0.006 * offline_minutes * 0.55, 0.0, 1.0)
    curiosity = clamp(curiosity - 0.003 * offline_minutes * 0.60, 0.0, 1.0)
    energy = clamp(energy + offline_minutes * 0.0015, 0.0, 1.0)
...
func _generate_personality() -> Dictionary:
    return {
        "mischief": randf_range(0.25, 0.95),
        "affection": randf_range(0.45, 0.95),
        "energy": randf_range(0.40, 0.95),
        "curiosity": randf_range(0.45, 0.95),
        "stubbornness": randf_range(0.15, 0.90),
